...
def _check_processing_summary(result: dict[str, object], known: dict[str, object]) -> pd.DataFrame:
    ...
    assert (summary["epoch_window_sec"] == 6.0).all()
    assert (summary["fft_crop_start_sec"] == 1.0).all()
    assert (summary["fft_crop_end_sec"] == 0.0).all()
    assert (summary["analysis_window_sec"] == 5.0).all()
    for code in config.ACTIVE_EVENT_CODES:
...
def _check_participant_spectra(
    ...
    expected_frequencies = np.fft.rfftfreq(1280, 1 / 256)
    np.testing.assert_allclose(cue_11.spectrum.freqs, expected_frequencies)
    assert cue_11.spectrum.amplitude_uv.shape == (64, 641)
...
def test_real_batch_workers_write_consolidated_group_outputs_and_keep_reruns_separate(
    ...
    assert len(participant_frame) == 2 * (1 + len(known["active_counts"])) * 641
    ...
    assert len(participant_11) == 641
    np.testing.assert_allclose(
        participant_11.frequency_hz,
        np.fft.rfftfreq(1280, 1 / 256),
    )
    ...
    assert len(group_frame) == (1 + len(known["active_counts"])) * 641
